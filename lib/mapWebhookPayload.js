...
// Reads the hour-of-day in the business's own timezone, not the server's —
// a Vercel/Railway server runs in UTC, so `Date#getHours()` on a UTC
// timestamp silently computes "after hours" against the wrong clock.
function getHourInBusinessTimezone(date) {
...
//     test_mode, call_summary, call_successful, detailed_transcript } }
...
function mapWebhookPayload(payload) {
  ...
  // call timestamp against business hours, evaluated in the business's own
  // timezone (see BUSINESS_TIMEZONE in lib/config.js) — not the server's.
  const hour = getHourInBusinessTimezone(new Date(callTimestamp));
  const derivedAfterHours = hour < BUSINESS_HOURS_START || hour >= BUSINESS_HOURS_END;
  ...
  const fields = {
          ...
          // Prefer the detailed Post-Call Data extraction field (richer, multi-sentence
          // recap) over the short built-in `summary`, if Ulio sent it.
          [CALL_LOG_FIELDS.transcript]: data.detailed_transcript || data.transcript || data.full_transcript || data.call_summary || data.summary || '',
  };
