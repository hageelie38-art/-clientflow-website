...
try {
  ...
  // Pass the full envelope (not just `payload.data`) so mapWebhookPayload
  // can read the top-level `timestamp` as well as the nested call data.
  const fields = mapWebhookPayload(payload);
